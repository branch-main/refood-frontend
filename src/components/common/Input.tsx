export const Input = ({ 
  ...
  return (
    <div className={`flex flex-col gap-2 ${fullWidth ? 'w-full' : ''}`}>
      {label && <label className="text-[0.9375rem] font-semibold text-gray-700 tracking-wide">{label}</label>}
      <input 
        type={type}
        ...
        {...props}
      />
      {error && <span className="text-red-600 text-sm mt-1 font-medium">{error}</span>}
    </div>
  );
};
